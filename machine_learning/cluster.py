...
"""
from machine_learning.base import ClusterModel
from machine_learning.distance import get_distance_metric
import numpy as np
import random
...
def _k_init(X, n_clusters, distance_metric):
    ...
    for i in range(1, n_clusters):
        ...
        for j in range(X_copy.shape[0]):
            nearest_distance = np.min([distance_metric(X_copy[j], centers[k])
                                       for k in range(i)])
...
def _gaussian_function(x, mu, sigma):
    ...
    if isinstance(mu, list) == list: mu = np.array(mu)
    if isinstance(sigma, list) == list: sigma = np.array(sigma)
    assert isinstance(x, np.ndarray) and isinstance(mu, np.ndarray) \
           and isinstance(sigma, np.ndarray)

    n_features = x.shape[0]
    if mu.shape[0] != n_features:
        raise Exception(
            "Parameters error, the shape of `mu` is not %s"
            % n_features
        )
    if sigma.shape != (n_features, n_features):
        raise Exception(
            "Parameters error, the shape of `delta` is not (%s, %s)"
...
class GaussianMixtureModel(ClusterModel):
    ...
    def _train(self, X):
        ...
        mu = np.array([[random.random() for _ in range(n_features)]
        ...
        for i in range(max_iter):
            ...
            # Expectation step
            for j in range(n_samples):
                temp_list = [coef[_] * _gaussian_function(X[j], mu[_], sigma[_])
                             for _ in range(n_clusters)]
                for k in range(n_clusters):
            ...
            for k in range(n_clusters):
                ...
                sigma[k] = np.sum([matrix[j][k] * np.outer(X[j] - mu[k], X[j] - mu[k])
                                for j in range(n_samples)], axis=0) / sub_sum
